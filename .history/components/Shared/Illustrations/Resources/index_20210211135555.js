...
function Resources (props) {
  ...
  const animatedItems = [
    <Phone key="animated1" animation={descend} />,
    <MagnifyingGlass key="animated2" animation={descend} />,
    <TextBubble key="animated3" animation={descend} />,
    <TopCube key="animated4" animation={descend} />,
    <BottomCube key="animated5" animation={descend} />
  ]

  const springs = useSpring(
    animatedItems.length, // the number of springs to create
      animatedItems.map(() => {
          return {
            config: config.slow,
            from: {
