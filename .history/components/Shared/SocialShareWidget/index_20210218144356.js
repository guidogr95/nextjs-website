...
const WidgetContainer = ({ style, url, toggle, state }) => {
    return (
        <>
            <div className="social-share-wrapper" style={style} >
                <div className="toggle-wrapper" >
                    <button
                        className={`toggle ${!state ? 'inactive' : ''}`}
    ...
    return (
        <>
            <Transition
                ...
                enter={{ left: '0px' }}
                leave={{ left: '-50px' }}>
                {show => show && (style => <WidgetContainer style={style} url={url} toggle={setActive} state={active} />)}
            </Transition>
        </>
    )
}
