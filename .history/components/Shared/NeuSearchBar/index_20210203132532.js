...
const NeuSearchBar = ({ placeholder, onSubmit, onChange, formData }) => {
    ...
    useEffect(() => {
        const inputFocus = () => {
            finder.current.classList.add('active')
        }

        const inputBlur = () => {
            if (input.current.value.length === 0) {
                finder.current.classList.remove('active')
            }
        }

        const formSubmit = (ev) => {
            ev.preventDefault()
            finder.current.classList.add('processing')
            finder.current.classList.remove('active')
            input.current.disabled = true
            setTimeout(() => {
                finder.current.classList.remove('processing')
        ...
        input.current.addEventListener('focus', inputFocus)
        input.current.addEventListener('blur', inputBlur)
        form.current.addEventListener('submit', formSubmit)
        ...
        <>
            <div className="sb-container">
                <form
                    ...
                    ref={form}
                >
                    <div className="finder" ref={finder} >
