...
const NeuSearchBar = ({ placeholder, onSubmit, onChange, formData, onFocus }) => {
    ...
    const [finderInputID, finderID, formID] = uuid().split('-')

    useEffect(() => {
        const input = document.querySelector(`.js${finderInputID}`)
        const finder = document.querySelector(`.js${finderID}`)
        const form = document.querySelector(`.js${formID}`)
        const inputFocus = () => {
        ...
        input && input.addEventListener('focus', inputFocus)
        input && input.addEventListener('blur', inputBlur)
        form && form.addEventListener('submit', formSubmit)

        return () => {
            input && input.removeEventListener('focus', inputFocus)
            input && input.removeEventListener('blur', inputBlur)
            form && form.removeEventListener('submit', formSubmit)
        }
    }, [])
        ...
        <>
            <div className="sb-container">
                <form
                    ...
                    className={`js${formID}`}
                >
                    <div className={`finder js${finderID}`}>
                    <div className="finder__outer">
                        <div className="finder__inner">
                            <div className="finder__icon" />
                            <input
                                className={`finder__input js${finderInputID}`}
