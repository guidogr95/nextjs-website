    ...
    const toggleMenu = (e) => {
        ...
        if (e.target.classList.contains('submenu')) {
            ...
            for (const i in siblings) {
                if (i !== 0) {
                    siblings[i].style.maxHeight = `${siblings[i].scrollHeight}px`
                }
            }
        } else {
                    ...
                    <>
                        ...
                        <div className="dropdown" id={`dropdown${index + 1}`} >
                            {submenu.map((item, index) => {
                                ...
                                return (
                                    <ul key={index} >
                                        {item.Label && <div className="submenu-label" >{item.Label} <FiChevronDown className="mobile-arrow submenu" onClick={toggleMenu} /></div>}
                                        {item.SubMenuItem.map((subItem, i) => {
                                            ...
                                            return (
                                                <li key={i} className={item.Label ? 'list' : ''} >
