        ...
        <>
            <li className="menuItem-parent" id={`navItem${index + 1}`} >
                ...
                { (submenu && submenu?.length > 0) &&
                    <>
                        <div className="mobile-arrow" onClick={toggleMenu} ><FiChevronDown onClick={(e) => e.stopPropagation()} /></div>
                        <div className="dropdown" id={`dropdown${index + 1}`} >
                            {submenu.map((item, index) => {
                                if (!item?.SubMenuItem || !item?.SubMenuItem?.length > 0) return null
                                return (
                                    <ul key={index} >
                                        {item.Label && <div className="submenu-label" >{item.Label} <div className="mobile-arrow submenu" onClick={toggleMenu} ><FiChevronDown /></div></div>}
